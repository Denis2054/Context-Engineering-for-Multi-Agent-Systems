...
def initialize_nim_clients(verbose: bool = True):
    ...
    On failure, returns (None, None, None) after printing the cause.
    """
    ...
    try:
        ...
        print("Required secrets: NVIDIA_API_KEY, API_KEY, PINECONE_API_KEY")
...
def check_index(pinecone_client, index_name: str, expected_dim: int,
    ...
    for name in required_namespaces:
        count = (ns.get(name) or {}).get("vector_count", 0)
        if count > 0:
