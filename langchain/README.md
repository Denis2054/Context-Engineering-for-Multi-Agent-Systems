...
`check_index()` also detects **near-miss namespaces** — names differing from an
engine namespace only by case, hyphens or underscores. Pinecone namespaces are
case-sensitive, so an index holding both `ContextLibrary` and `context-library`
will serve the engine whichever one `CONFIG` names, silently and without error.
When the near-miss holds more vectors than the namespace the engine reads, the
pre-flight fails rather than warns: reading a stale or partial library is not a
condition to proceed from.
