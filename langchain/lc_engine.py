...
PLANNER_PROMPT = ChatPromptTemplate.from_messages([
    ("system",
     ...
     "rest null.\n"
     "4. BLUEPRINT FIRST — this rule is not optional. If the goal asks for "
     ...
     "5. A goal that only asks to retrieve, summarize, extract or answer — with "
     "no styled deliverable — does not need the Librarian.\n"
     "6. Insert a Summarizer step between the Researcher and the Writer when the "
...
def validate_plan(plan: List[dict], goal: str = "") -> List[str]:
    ...
    if any(v in lowered for v in STYLED_OUTPUT_VERBS) and "Writer" not in agents:
        findings.append(
