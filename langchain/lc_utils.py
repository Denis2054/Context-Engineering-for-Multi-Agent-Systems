...
def check_index(index_name: str = EXPECTED_INDEX,
    ...
    # Near-miss detection. A namespace whose name differs from an engine
    # namespace only by case, hyphens or underscores is almost always the same
    # data written by a different notebook. Pinecone namespaces are
    # case-sensitive, so this misconfiguration fails SILENTLY: the engine reads
    # a valid but wrong namespace and returns confident, irrelevant results.
    def _key(name: str) -> str:
        return "".join(ch for ch in name.lower() if ch.isalnum())

    expected_keys = {_key(n): n for n in namespaces}
    for other, count in sorted(others.items(), key=lambda kv: -kv[1]):
        match = expected_keys.get(_key(other))
        if not match:
            continue
        engine_count = (ns_stats.get(match) or {}).get("vector_count", 0)
        print(f"   [WARN ] namespace '{other}' ({count} vectors) differs from "
              f"the engine's '{match}' ({engine_count} vectors) only by case or "
              f"punctuation.")
        if count > engine_count:
            print(f"            '{other}' holds MORE vectors than the namespace "
                  f"this engine reads. If it is the current library, the engine "
                  f"is reading stale or partial data.")
            print("            Confirm which namespace your ingestion notebooks "
                  "write to, then align lc_helpers.CONFIG.")
            ok = False
