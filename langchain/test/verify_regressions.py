...
# =============================================================================
section("R10. Pre-flight severity — advisories must never block")
# =============================================================================
# A near-miss namespace heuristic was once allowed to set the pre-flight to
# FAIL. It blocked a perfectly runnable index and printed "Populate the index"
# advice that did not apply. The rule established here: only conditions that
# make the engine UNABLE TO RUN may block. Everything else reports.
...
class _FakePinecone:
    ...
    def Index(self, name):
        return _FakeIndex(_FakePinecone.stats)


_pc = _types.ModuleType("pinecone")
_pc.Pinecone = _FakePinecone
sys.modules["pinecone"] = _pc
os.environ.setdefault("PINECONE_API_KEY", "regression-test")

import contextlib as _ctx
import io as _io


def _preflight(stats):
    _FakePinecone.stats = stats
    buf = _io.StringIO()
    with _ctx.redirect_stdout(buf):
        result = lc_utils.check_index()
    return result, buf.getvalue()

# The operator's real index: engine namespaces healthy, four foreign namespaces,
# two of which differ only by case.
REAL_INDEX = {
    "dimension": 1536, "total_vector_count": 133,
    "namespaces": {
        "ContextLibrary": {"vector_count": 3}, "KnowledgeStore": {"vector_count": 10},
        "context-library": {"vector_count": 49}, "mgny-blueprints": {"vector_count": 49},
        "mgny-applicants": {"vector_count": 13}, "knowledge-store": {"vector_count": 9}},
}
ok_real, out_real = _preflight(REAL_INDEX)
ledger("a healthy index with foreign near-miss namespaces PASSES pre-flight",
       ok_real, out_real)
ledger("no false 'Populate the index' advice on a populated index",
       "Populate the index" not in out_real)
ledger("near-miss is a NOTE, never a FAIL", "[NOTE ]" in out_real
       and "[FAIL ]" not in out_real)
ledger("thin blueprint library warns without blocking",
       "only 3 blueprint(s)" in out_real and ok_real)

ok_empty, out_empty = _preflight({
    "dimension": 1536, "total_vector_count": 3,
    "namespaces": {"ContextLibrary": {"vector_count": 3}}})
ledger("an empty engine namespace still blocks", not ok_empty)
ledger("ingestion advice appears only for an empty namespace",
       "Populate the index" in out_empty)

ok_dim, out_dim = _preflight({
    "dimension": 3072, "total_vector_count": 13,
    "namespaces": {"ContextLibrary": {"vector_count": 3},
                   "KnowledgeStore": {"vector_count": 10}}})
ledger("a dimension mismatch still blocks", not ok_dim)
ledger("remediation matches the cause, not a generic message",
       "different embedding model" in out_dim and "Populate the index" not in out_dim)
