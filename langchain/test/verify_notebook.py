...
check("namespace accounting is explained",
      "wrote elsewhere in the same index" in all_text)
check("pre-flight severity is documented",
      "advisory and do not stop the run" in all_text)
check("near-miss is documented as non-blocking",
      "Informational, not a fault, and it does not block" in all_text)
