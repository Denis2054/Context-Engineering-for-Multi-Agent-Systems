...
# A NAMING heuristic must never block a runnable index. describe_index_stats()
# cannot tell "the operator mistyped a namespace" from "another project shares
# this index", so this reports and the operator decides.
check("a near-miss NEVER fails the pre-flight when the engine namespaces are healthy",
      near_ok, near)
check("near-miss is reported as a NOTE, not a WARN or FAIL",
      "[NOTE ] 'context-library'" in near)
check("near-miss advisory says it is not a fault",
      "informational, not a fault" in near)
check("both near-misses are reported regardless of relative size",
      "'knowledge-store' (9 vectors)" in near and "'context-library' (49 vectors)" in near)
check("unrelated namespaces are not flagged as near-misses",
      "'mgny-blueprints' (49 vectors) differs" not in near)
check("a healthy index says so explicitly", "Pre-flight OK" in near)
check("no ingestion advice is printed when nothing is empty",
      "Populate the index" not in near, near)
...
dim_report = buf.getvalue()
check("dimension mismatch is explained", "expected 1536" in dim_report)
check("dimension remediation is printed", "different embedding model" in dim_report)
check("ingestion advice is not printed for a dimension failure",
      "Populate the index" not in dim_report, dim_report)
...
empty_report = buf.getvalue()
check("empty namespace names the ingestion fix", "clear_index=False" in empty_report)
check("empty-namespace remediation names WHICH namespace",
      "'KnowledgeStore'" in empty_report, empty_report)
check("dimension advice is not printed for an empty-namespace failure",
      "different embedding model" not in empty_report)
