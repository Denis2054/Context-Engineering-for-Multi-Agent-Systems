...
def blueprint_diagnostics(context_store, queries, k: int = 3) -> bool:
    """
    Show which blueprints a set of intent phrases actually retrieves, with
    similarity scores. Returns True if every query's top hit is a clear winner.

    Why this exists: the Librarian runs at k=1, so it returns exactly one
    blueprint and gives no indication of how close the runner-up was. Over a
    small ContextLibrary the nearest neighbour can be effectively arbitrary — a
    request for "a formal structured legal summary" can return a casual
    blueprint, and the Writer will then faithfully produce casual prose. The
    engine is working; the retrieval is not. This makes that visible before a
    demo rather than during one.

    A margin below 0.05 between the top two hits means the choice is close to a
    coin flip and the library needs more differentiated blueprints.
    """
    clean = True
    for query in queries:
        try:
            hits = context_store.similarity_search_with_score(query, k=k)
        except Exception as e:
            print(f"Could not run blueprint diagnostics: {e}")
            return False

        print(f'\nIntent: "{query}"')
        if not hits:
            print("   (no blueprints returned)")
            clean = False
            continue

        for rank, (doc, score) in enumerate(hits, start=1):
            label = doc.metadata.get("description") or doc.metadata.get("id") or "—"
            preview = " ".join((doc.page_content or "").split())[:90]
            marker = "->" if rank == 1 else "  "
            print(f"   {marker} {score:.4f}  {label[:40]:<40} {preview}")

        if len(hits) > 1:
            margin = abs(hits[0][1] - hits[1][1])
            if margin < 0.05:
                clean = False
                print(f"      [WARN ] margin over runner-up is only {margin:.4f} — "
                      f"this selection is close to arbitrary.")
    if not clean:
        print("\n   Blueprint retrieval is weak. Either ingest more clearly "
              "differentiated blueprints,\n   or raise k_blueprint and let the "
              "Writer choose. Retrieval quality is an\n   ingestion problem, not "
              "an engine problem.")
    return clean
